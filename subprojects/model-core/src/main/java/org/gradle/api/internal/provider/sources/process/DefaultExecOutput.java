...
public class DefaultExecOutput implements ExecOutput {
    ...
    public Provider<ExecResult> getResult() {
        return dataProvider.map((Transformer<ExecResult, ExecOutputData> & Serializable) ExecOutputData::getResult);
    }

    @Override
    public StandardStreamContent getStandardOutput() {
        return new DefaultStandardStreamContent(dataProvider.map((Transformer<byte[], ExecOutputData> & Serializable) ExecOutputData::getOutput));
    }

    @Override
    public StandardStreamContent getStandardError() {
        return new DefaultStandardStreamContent(dataProvider.map((Transformer<byte[], ExecOutputData> & Serializable) ExecOutputData::getError));
    }
    ...
    private static class DefaultStandardStreamContent implements StandardStreamContent {
        ...
        public Provider<String> getAsText() {
            return getAsBytes().map((Transformer<String, byte[]> & Serializable) bytes -> new String(bytes, Charset.defaultCharset()));
        }
